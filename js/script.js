...
// Comportamiento base
const MAX_ENEMIES_BASE = 9;            // máximo de enemigos en pantallas (valor base)
const ENEMY_SPAWN_INTERVAL_BASE = 1800; // intervalo de aparición base (ms)
const BULLET_SPEED = 18;               // velocidad de las balas del jugador
...
enemyImage.src = "img/avion1.png"; // imagen del enemigo 

const tankImage = new Image();
tankImage.src = "img/avion2.png";  // imagen del jugador
...
function drawTank(t) {
  ...
  if (tankImage.complete && tankImage.naturalWidth > 0) {
    ...
    const ancho = tankImage.width * 0.15;
    const alto = tankImage.height * 0.15;
...
function drawEnemy(e){
  ...
  if (enemyImage.complete && enemyImage.naturalWidth > 0) {
    ...
    const w = enemyImage.width * 0.15;
    const h = enemyImage.height * 0.15;
...
function drawBullets(){
  ...
  for(const b of bullets){
    ctx.beginPath();
    ctx.moveTo(b.x - b.vx*0.15, b.y - b.vy*0.15);
...
function render(dt){ 
  ...
  // mensaje de nivel (si corresponde)
  drawLevelMessage(dt);
}
